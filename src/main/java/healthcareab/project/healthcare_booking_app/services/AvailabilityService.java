...
public class AvailabilityService {
    ...
    public AvailabilityResponse createAvailability(AvailabilityRequest dtoRequest) {
        ...
        Caregiver caregiver = userRepository.findById(dtoRequest.getCaregiverId()).filter(Caregiver.class::isInstance)
                .map(Caregiver.class::cast).orElseThrow(() -> new ResourceNotFoundException("Caregiver not found"));
    ...
    public AvailabilityResponse getAvailabilityById(Long id) {
        Availability availability = availabilityRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("No availability found"));
        return mapToResponse(availability);
    }
    ...
    public List<AvailabilityResponse> getAllAvailabilities() {
        ...
        return availabilityRepository.findAll().stream().map(this::mapToResponse).toList();
    }
    ...
    private AvailabilityResponse mapToResponse(Availability availability) {
        return new AvailabilityResponse(availability.getId(), availability.getStartTime(), availability.getEndTime(),
                availability.isReoccurring(), availability.getCaregiver().getId(), availability.getCreatedAt());
    }
